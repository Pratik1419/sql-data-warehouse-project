...
BEGIN
    ...
    BEGIN TRY
        ...
        )t WHERE flag_last = 1;    -- select the most recnt record per customer.
        ...
        SELECT 
	        ...
	        CAST (LEAD(prd_end_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) AS prd_end_dt -- Calculation end date as one day before the next start date.
